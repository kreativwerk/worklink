  ...
  return (
    <div className={cn("flex flex-col gap-1.5", className)}>
      <label htmlFor={htmlFor} className="text-sm font-medium">
        {label}
        {required && <span className="ml-0.5 text-lime-2">*</span>}
      </label>
      {children}
...
export function Button({
  ...
  return (
    <button
      {...props}
      className={cn(
        "inline-flex items-center justify-center gap-2 rounded-full px-6 py-3 text-[15px] font-semibold transition-all disabled:cursor-not-allowed disabled:opacity-50",
        variant === "primary" &&
          "bg-lime text-on-lime hover:opacity-90 active:scale-[0.98]",
        variant === "ghost" && "border border-border text-fg hover:bg-fg/5",
        className,
      )}
    />
  );
